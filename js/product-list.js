const displayAllProducts = () => {
  $.get("DB/product-list.php?type=all").done((data) => {
    let json = JSON.parse(data);
    json.results.forEach((val) => {
      $.post("components/product-card.php", val).done((res) => {
...
const displayCarousel = () => {
  $.get("DB/product-list.php?type=all").done((data) => {
    ...
    json.results.forEach((val, i) => {
      ...
      $.post("components/carousel-item.php", val).done((res) => {
...
const displayCategory = (type) => {
  $.get(`DB/product-list.php?type=${type}`).done((data) => {
    let json = JSON.parse(data);
...
const highlightProduct = (product_ID) => {
  $.get(`DB/product-list.php?type=one&product_ID=${product_ID}`).done((res) => {
    let data = JSON.parse(res);
    $.post(`components/product-highlight.php`, data.results[0]).done((res) => {
