const addToCart = (user_ID, product_ID, variants_ID) => {
  ...
  $.post("DB/add-to-cart.php", data).done((res) => {
    console.log(res);
    M.toast({ html: res });
    getCart(user_ID);
  });
...
const getCart = (user_ID) => {
  if (user_ID != "null") {
    ...
    $("#cart-list").html("");
    $.get(`DB/add-to-cart.php?type=get&user_ID=${user_ID}`).done((res) => {
      ...
      data.result.forEach((val) => {
        $.post("components/cart-item.php", val).done((htmlRes) => {
...
const displayCheckout = (user_ID) => {
  ...
  $.post("DB/add-to-cart.php", data).done((res) => {
    ...
    json.forEach((val, i) => {
      ...
      $.post("components/checkout-item.php", val).done((res) => {
...
const displayTotalCheckout = (user_ID) => {
  ...
  $.post("DB/add-to-cart.php", data).done((res) => {
    ...
    $.post("components/checkout-item.php", json).done((res) => {
      $("#checkout-data").append(res);
    });

    $("#merchandise-price-payment").val(parseFloat(json.final_price));
    $("#total-quantity").val(parseFloat(json.quantity));
...
const checkOutItems = (input_data, user_ID) => {
  ...
  $.post("DB/checkout.php", data).done((res) => {
    if (res == "success") {
