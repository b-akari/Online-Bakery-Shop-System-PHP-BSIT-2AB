<!DOCTYPE html>
<html>

<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <title>Cookies All Products</title>
  <link rel="stylesheet" type="text/css" href="style.css">
  <link rel="preconnect" href="https://fonts.googleapis.com">
  <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
  <link
    href="https://fonts.googleapis.com/css2?family=Poppins:ital,wght@0,200;0,400;0,500;0,600;0,700;1,300&display=swap"
    rel="stylesheet">
  <link rel="stylesheet" type="text/css" href="fontawesome-free-6.1.1-web/css/all.min.css">
</head>

<body>
<<<<<<< Updated upstream
	<!-- navigation bar -->
	<?php include 'components/navbar.php' ?>
=======
  <!-- navigation bar -->
  <?php include 'components/navbar.php' ?>
>>>>>>> Stashed changes


  <div class="menu">
    <div class="cookie">
      <h2>COOKIE</h2>
      <div class="col-2">
        <img src="images/chocochip_cookie.png">
        <div class="middle">
          <a href="product.php?name=chocochip" class="text2">Add to Cart</a>
        </div>
      </div>
      <div class="col-2">
        <img src="images/chococringles_cookie.png">
        <div class="middle">
          <a href="product.php?name=chocoringles" class="text2">Add to Cart</a>
        </div>
      </div>
      <div class="col-2">
        <img src="images/gingerbread_cookie.png">
        <div class="middle">
          <a href="product.php?name=gingerbread" class="text2">Add to Cart</a>
        </div>
      </div>
    </div>
    <br><br><br><br><br><br><br><br><br><br>

<<<<<<< Updated upstream
	<!-- footer -->
	<?php include 'components/footer.php' ?>

	<script src="menu.js"></script>
=======
  </div>

  <!-- footer -->
  <?php include 'components/footer.php' ?>

  <script src="menu.js"></script>
>>>>>>> Stashed changes
</body>

</html>